...
| Sightings log | Arrival and departure transitions of each indicator per feed, with a timestamp. Used for churn statistics and the optional predictor | `sightings` in `threatfeedme-churn.db` (a second file beside the database since v2.5.0, backed up with it), `source_state` | Transitions only, bounded by the live corpus. Feeds listed under `retention.churn_log_exclude` are not logged. |
