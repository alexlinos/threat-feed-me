"""Host-header allowlist (v2.5.0) against DNS rebinding: report-only until an
allowlist exists, IPs always allowed, feeds never checked."""
import pytest
...
def test_enforcing_refuses_unknown_names():
    c = _client(["threatfeedme.tnh.local"])
...
def test_ip_literals_and_localhost_always_work(host):
    ...
    assert _get(_client(["only.example"]), "/api/stats", host).status_code == 200
...
def test_feeds_and_healthz_are_never_checked(path):
    ...
    assert _get(_client(["only.example"]), path, "attacker.example").status_code == 200
